...
public:
    ...
    /// Index
    enum {
        IDX_MIRROR=0, IDX_SCALEPRIOR, IDX_BIAS, IDX_DEF, IDX_APP, IDX_FILTER, IDX_AOGRID
#if RGM_USE_PCA_DIM
        , IDX_WPCA, IDX_PCAFILTER
#endif
        , IDX_NUM
    };
...
#if RGM_USE_PCA_DIM
    /// Type of FFT PCA filter
    typedef Patchwork::PCAFilter FFTPCAFilter;
#endif
...
#if RGM_USE_PCA_DIM
    /// Returns the PCA param
    const Appearance::PCAParam * wpca() const;
    Appearance::PCAParam *& getWpca();

    /// Returns the PCA FFT filter
    const FFTPCAFilter *  cachedFFTPCAFilter() const;
    FFTPCAFilter *& getCachedFFTPCAFilter();
#endif
...
#if RGM_USE_PCA_DIM
    Appearance::PCAParam * wpca_;
    FFTPCAFilter * cachedFFTPCAFilter_;
#endif
...
public:
...
#ifdef RGM_USE_MATLAB
    /// Converts from a DPM VOC-Release5 model
    void cvtVOCRel5Model(const std::string & modelFile);        
#endif
    ...
    bool isSingleObjModel() const;
    bool & getIsSingleObjModel();
...
#if RGM_USE_PCA_DIM
    /// Projects the model appearance parameters
    /// @param[in] pcaCoefFile The precomputed pca data (NbFeature-1 * NbFeature-1 matrix)
    ///  and we assume it is stored in row-major order
    bool project(const std::string & pcaCoefFile);

    /// Returns pca coef. matrix
    const Matrix * pcaCoef() const;
    Matrix *& getPcaCoef();

    /// Returns appearance pca paramters
    const std::vector<Appearance::PCAParam *>  & pcaAppearanceParam() const;
    std::vector<Appearance::PCAParam *>  & getPcaAppearanceParam();

    /// Returns the cached FFT filters
    const std::vector<Node::FFTPCAFilter *> &  cachedFFTPCAFilters() const;
    std::vector<Node::FFTPCAFilter *> &  getCachedFFTPCAFilters();

    /// Transfers the PCA filters of T-nodes
    void cachingFFTPCAFilters();
    ...
    const std::map<int, Matrix> & bboxPred() const;
    std::map<int, Matrix> & getBboxPred();
...
#if RGM_USE_PCA_DIM
    /// Returns the index of a pca appearance param
    int idxWpca(const Appearance::PCAParam *  w) const;

    /// Returns the index of a pca fft filter
    int idxFFTPCAFilter(const Node::FFTPCAFilter *  filter) const;
#endif
...
#if RGM_USE_PCA_DIM
    /// Returns the PCA-projected appearance with the given index
    Appearance::PCAParam * findWpca(int idx);

    /// Returns the fft pca filter with the given index
    Node::FFTPCAFilter *  findCachedFFTPCAFilter(int idx);
#endif
...
private:
    ...
    std::map<int, Matrix> bboxPred_; //(nodeId, predModel) for each object component, 4-column matrix (x1, y1, x2, y2)
...
#if RGM_USE_PCA_DIM
    Matrix * pcaCoef_;
    std::vector<Appearance::PCAParam *>  pcaAppearanceParam_;
    std::vector<Node::FFTPCAFilter *>  cachedFFTPCAFilters_;
#endif
