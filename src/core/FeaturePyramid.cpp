    ...
    cellSize_(0), padx_(0), pady_(0), interval_(0), octave_(0), extraOctave_(false)
{
    if ( (!extraOctave && cellSize/2<2) || (extraOctave && cellSize/4<2)  ||
         ...
         levels.size() != PCAlevels.size() || levels.size() != scales.size() ) {
...
#if RGM_USE_PCA_DIM
const std::vector<FeaturePyramid::PCALevel> & FeaturePyramid::PCAlevels() const
{
    return PCAlevels_;
}

void FeaturePyramid::project(const Matrix & coef)
{
    // coef: NbPCAFeature * NbFeatures matrix
    RGM_CHECK_EQ(coef.rows(), NbPCAFeatures);
    RGM_CHECK_EQ(coef.cols(), NbFeatures);

    PCAlevels_.resize( nbLevels() );

    const std::vector<Level> & L(levels());

#pragma omp parallel for
    for ( int l = 0; l < nbLevels(); ++l ) {

        const Level & curL( L[l] );
        PCALevel & curPCAL( PCAlevels_[l] );
        curPCAL.resize(curL.rows(), curL.cols());

        for ( int r = 0; r < curL.rows(); ++r ) {
            for ( int c = 0; c < curL.cols(); ++c ) {

                curPCAL(r, c) = coef * curL(r, c).matrix();
            }
        }
    }
}
...
#if RGM_USE_PCA_DIM
    ar & BOOST_SERIALIZATION_NVP(PCAlevels_);
