function convertVOCR5()
% Add a DFS field to the model structure such that it can be converted to our AOGrammar

% modify them accordingly
modelDir = '/home/tfwu/Dropbox/RGM-Release/preTrainedModel/voc2007_DPM/'; % directory path to your pretrained models using voc-r5;

objNames = {'person', 'bicycle','aeroplane','car','boat', 'bottle','bus','bird','cat','chair','cow',...
    'diningtable','dog','horse','motorbike','pottedplant','sheep','sofa',...
    'train','tvmonitor'};

for i = 1 : length(objNames)
    modelFile = sprintf('%s%s_final.mat', modelDir, objNames{i}); 
    load(modelFile);
    
    model.DFS = model_sort(model);
    
    save(modelFile, 'model');
end


function [L, V] = model_sort(m, i, L, V)
% AUTORIGHTS
% -------------------------------------------------------
% Copyright (C) 2011-2012 Ross Girshick
%
% This file is part of the voc-releaseX code
% (http://people.cs.uchicago.edu/~rbg/latent/)
% and is available under the terms of an MIT-like license
% provided in COPYING. Please retain this notice and
% COPYING if you use this file (or a portion of it) in
% your project.
% -------------------------------------------------------

% Topological sort of the nonterminal symbols in m's grammar.
%   [L, V] = model_sort(m, i, L, V)
%
% Return values
%   L   Symbols visited in post order
%   V   (internal use) Symbol visitation status
%
% Arguments
%   m   Object model
%   i   (internal use) Current symbol
%   L   (internal use) Symbols visited thus far in post order
%   V   (internal use) Symbol visitation status thus far

% AUTORIGHTS
% -------------------------------------------------------
% Copyright (C) 2009-2012 Ross Girshick
%
% This file is part of the voc-releaseX code
% (http://people.cs.uchicago.edu/~rbg/latent/)
% and is available under the terms of an MIT-like license
% provided in COPYING. Please retain this notice and
% COPYING if you use this file (or a portion of it) in
% your project.
% -------------------------------------------------------

% initialize depth-first search at start symbol
if nargin < 2
    i = m.start;
    L = [];
    V = zeros(m.numsymbols, 1);
end

% check for cycle containing symbol i
if V(i) == 1
    error('Cycle detected in grammar!');
end

% mark symbol i as pre-visit
V(i) = 1;
for r = 1:length(m.rules{i})
    for s = m.rules{i}(r).rhs
        % recurse if s is a nonterminal and not already visited
        if V(s) < 2 %m.symbols(s).type == 'N' && V(s) < 2
            [L, V] = model_sort(m, s, L, V);
        end
    end
end
% mark symbol i as post-visit
V(i) = 2;
L = [L i];
